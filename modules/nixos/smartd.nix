# copy of https://github.com/NixOS/nixpkgs/blob/nixos-24.05/nixos/modules/services/monitoring/smartd.nix with extension of ntfy
# TODO: upstream
{
  config,
  lib,
...
with lib; let
  ...
  smartdNotify = pkgs.writeScript "smartd-notify.sh" ''
    ...
    ''}
    ${optionalString ntfy.enable ''
      ${pkgs.curl}/bin/curl -Ls \
            -u laptop:$(${pkgs.coreutils}/bin/cat ${config.sops.secrets."ntfy".path}) \
            -H prio:high \
            -H "Title: $SMARTD_SUBJECT" \
            -d "$SMARTD_FAILTYPE Device: $SMARTD_DEVICE Time: $SMARTD_TFIRST Message: $SMARTD_FULLMESSAGE" \
            https://ntfy.weriomat.com/zfs
    ''}
  ...
  notifyOpts =
    optionalString (nm.enable || nw.enable || nx.enable)
      ...
      notifications = {
        ...
        ntfy = {
          enable = mkOption {
            default = true;
            type = types.bool;
            description = "Whenever to send ntfy";
          };
        };
